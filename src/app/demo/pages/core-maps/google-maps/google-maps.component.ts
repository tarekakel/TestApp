...
export class GoogleMapsComponent implements OnInit {
  ...
  constructor(private vehicleService: VehicleService,public mapsAPILoader:MapsAPILoader,private route: ActivatedRoute) {
  ...
  getAllData(setPageFlag){
     

    this.subscription = timer(0,30000).pipe(switchMap(()=>this.vehicleService.getAllVehicleLocations())).subscribe(res => {
        ...
        for (var i = 0; i < this.vehiclesInfo.length; i++) {
            ...
            else if (this.vehiclesInfo[i].idle == false &&
            ...
            } else if (this.vehiclesInfo[i].reporting == true && this.vehiclesInfo[i].engineOn == false && 
              this.vehiclesInfo[i].vehicleStatus == "Active" && this.vehiclesInfo[i].idle == true) {
                this.redVehiclesInfo.push(this.vehiclesInfo[i]);     // stopped

            }else if (this.vehiclesInfo[i].engineOn == true && this.vehiclesInfo[i].idle == true
              && this.vehiclesInfo[i].reporting ==true && this.vehiclesInfo[i].vehicleStatus == "Active") {
        ...
        this.tableVehiclesInfo = this.tableVehiclesInfo.concat(this.greyVehiclesInfo);
        this.tableVehiclesInfo = this.tableVehiclesInfo.concat(this.redVehiclesInfo);
        this.tableVehiclesInfo = this.tableVehiclesInfo.concat(this.yellowVehilesInfo);       

...
import { switchMap, take, filter, catchError, tap, map } from 'rxjs/operators';
...
export class RequestInterceptorService implements HttpInterceptor {
    ...
    addToken(req: HttpRequest<any>, token: string): HttpRequest<any> {
        ...
        if (!req.url.includes('assets')) {//Only add token when call api not for every request like localization files
        ...
        return next.handle(this.addToken(req, this.authService.getToken())).pipe(
            tap(res => {
                if (res instanceof HttpResponse) {
                    ...
                    localStorage.setItem("lastActiveTime", res.headers.get('lastActiveTime'));
            ...
            catchError(error => {
                ...
                if (error instanceof HttpErrorResponse) {
                    ...
                    this.alertService.showAlertMessage("error", error, AlertMessageType.Error);
                }

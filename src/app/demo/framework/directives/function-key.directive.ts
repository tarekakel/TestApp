import { Directive, OnInit, HostListener, Input, TemplateRef } from '@angular/core';
...
export class FunctionKeyDirective implements OnInit {
  ...
  @Input() TemplateSize: string;
  @Input() TemplateToOpen: TemplateRef<any>;
  ...
  ngOnInit() {
    ...
    if (this.TemplateSize == '' || this.TemplateSize == null || typeof this.TemplateSize === 'undefined' ) {
  ...
  @HostListener('keydown', ['$event']) onKeyDown(event) {
    ...
    if (e.keyCode == 112 && this.FunctionKey == 'F1') {
      this.modalRef = this.modalService.show(this.TemplateToOpen,
